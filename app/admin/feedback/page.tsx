...
function AdminChatContent() {
  ...
  return (
    <div className="h-[calc(100vh-theme(spacing.16))] bg-slate-50 dark:bg-slate-950 p-6 flex flex-col">
      <div className="mb-4">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Chat with Learners</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1">Communicate directly with your learners.</p>
      </div>

      <div className="flex flex-1 gap-6 min-h-0 overflow-hidden">
        {/* Conversation List */}
        {!learnerIdParam && (
        <div className="w-1/3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {conversations.length === 0 ? (
               <div className="p-8 text-center text-slate-400 dark:text-slate-500">No conversations found.</div>
            ) : (
                ...
                >
                    ...
                    <h3 className="font-semibold text-slate-900 dark:text-white">{conv.learner?.fullName}</h3>
                    {conv.unreadCount > 0 && (
        ...
        {/* Chat Window */}
        <div className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col overflow-hidden relative">

          {showNewChat ? (
             <div className="flex-1 flex flex-col p-8">
                 <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">Start a New Chat</h2>
                    <button onClick={() => setShowNewChat(false)} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white">
                        Cancel
                    </button>
                 </div>
                 <div className="space-y-4">
                     <label className="block text-sm font-medium text-slate-600 dark:text-slate-300">Select Learner</label>
                     <select
                        value={selectedNewLearner}
                        onChange={(e) => setSelectedNewLearner(e.target.value)}
                        className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-4 py-3 text-slate-900 dark:text-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                     >
                     ...
                     return (
                        <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                          <div className={`max-w-[70%] rounded-2xl px-4 py-2 ${
                              isMe ? "bg-blue-600 text-white rounded-br-none" : "bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 rounded-bl-none"
                          }`}>
                             <p className="whitespace-pre-wrap">{msg.message}</p>
                             <span className={`text-[10px] mt-1 block ${isMe ? "text-blue-200" : "text-slate-400 dark:text-slate-500"}`}>
              ...
              <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50">
                <form onSubmit={sendMessage} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-4 py-2 text-slate-900 dark:text-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  />
                  <button
                    type="submit"
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 p-8">
               <svg className="w-16 h-16 mb-4 text-slate-300 dark:text-slate-700" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
...
export default function AdminFeedbackPage() {
   return (
      <Suspense fallback={<div className="flex items-center justify-center h-screen bg-slate-50 dark:bg-slate-950"><div className="animate-spin h-8 w-8 border-4 border-blue-500 border-t-transparent rounded-full" /></div>}>
         <AdminChatContent />
      </Suspense>
   );
}