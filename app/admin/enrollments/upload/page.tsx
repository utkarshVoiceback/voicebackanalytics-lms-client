...
function UploadContent() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-8">
        <Link href={initialBatchId ? `/admin/batches/${initialBatchId}` : "/admin/batches"} className="text-sm text-blue-400 hover:text-blue-300 transition-colors mb-2 inline-block">
          ← Back to Batch
        </Link>
        <h1 className="text-3xl font-bold text-white tracking-tight">Bulk Upload Enrollments</h1>
        <p className="text-slate-400 mt-1">Upload an Excel file to parse, validate, and securely invite learners</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-5">
            <div>
              <label htmlFor="formId" className="block text-sm font-medium text-slate-300 mb-1.5">
                Enrollment Form ID <span className="text-red-400">*</span>
              </label>
              <input
                id="formId"
                type="text"
                ...
                className={`w-full rounded-lg border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors ${initialFormId ? 'opacity-70 cursor-not-allowed' : ''}`}
              />
            </div>
            ...
            <div>
              ...
              <div className="mt-2 flex justify-center rounded-lg border border-dashed border-slate-700 px-6 py-10 hover:border-blue-500 hover:bg-slate-800/50 transition-colors relative">
                <div className="text-center">
                  ...
                  <p className="text-xs leading-5 text-slate-500 mt-2">Required Columns: Name, Email, Mobile</p>
            ...
            <button
              ...
              className="w-full flex justify-center items-center gap-2 rounded-lg bg-slate-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {loading ? (
                <>
        ...
        <div>
          {uploadResults ? (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 h-full flex flex-col">
              <h3 className="text-lg font-semibold text-white mb-4">Validation Preview</h3>
              <div className="grid grid-cols-3 gap-3 mb-6">
                ...
                <div className="bg-emerald-500/10 p-4 rounded-lg border border-emerald-500/20">
                  <p className="text-sm text-emerald-500/70">Valid</p>
                  <p className="text-2xl font-bold text-emerald-400">{uploadResults.valid}</p>
                </div>
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    {uploadResults.rows?.map((row: any, idx: number) => (
                      <tr key={idx} className={row.isValid ? "bg-slate-900/50" : "bg-red-950/20"}>
                        ...
                        <td className="px-4 py-3 text-red-400 text-xs truncate max-w-[150px]" title={row.error}>{row.error || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {sendResults ? (
                <div className="bg-emerald-500/10 border border-emerald-500/30 p-4 rounded-lg">
                  <h4 className="text-emerald-400 font-semibold mb-2">Invitations Processing Started</h4>
                  ...
                  <Link href={`/admin/enrollments`} className="mt-4 inline-block text-sm text-emerald-400 hover:text-emerald-300 underline">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 h-full flex flex-col items-center justify-center text-center opacity-50">
              ...
              <h3 className="text-lg font-medium text-slate-300">Preview Results</h3>
              <p className="text-sm text-slate-500 mt-1">Upload an Excel file to view validation details and send invitations.</p>
            </div>
          )}
        </div>
      </div>
