  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <Link
        ...
        className="text-sm text-blue-400 hover:text-blue-300 transition-colors mb-4 inline-block"
      >
        ← Back to Enrollment Forms
      </Link>

      <h1 className="text-3xl font-bold text-white tracking-tight mb-2">Create Enrollment Form</h1>
      <p className="text-slate-400 mb-8">Generate a new enrollment form link for a batch</p>
      ...
      {createdForm ? (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-6 mb-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="text-lg font-semibold text-emerald-400">Enrollment Form Created Successfully!</h2>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
                <p className="text-xs text-slate-500 mb-2 uppercase font-medium">Enrollment Link</p>
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    value={enrollmentLink}
                    readOnly
                    className="flex-1 bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm text-white font-mono overflow-auto"
                  />
                  <button
                    onClick={() => {
              ...
              <div className="bg-slate-800/30 rounded-lg p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-500">Form ID:</span>
                  <span className="text-white font-mono">{createdForm.id}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Status:</span>
                  <span className="text-emerald-400 font-medium">{createdForm.status}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Created:</span>
                  <span className="text-white">{new Date(createdForm.createdAt).toLocaleString()}</span>
                </div>
              </div>

              <p className="text-xs text-slate-500 mt-4">
                💡 You can now use this form ID in the <Link href="/admin/enrollments/upload" className="text-blue-400 hover:text-blue-300">Bulk Upload</Link> feature to invite multiple learners at once.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <Link
              href="/admin/enrollment-forms"
              className="flex-1 px-4 py-3 bg-slate-800 hover:bg-slate-700 text-white text-center font-semibold rounded-lg transition-colors"
            >
              Back to Forms
            </Link>
            <Link
        ...
        <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-2xl p-8 space-y-6">
          ...
          <div>
            ...
            {batchesLoading ? (
              <div className="animate-pulse bg-slate-800 h-10 rounded-lg"></div>
            ) : batches.length === 0 ? (
              <p className="text-slate-400 text-sm">
                No active batches available. <Link href="/admin/batches/create" className="text-blue-400 hover:text-blue-300">Create a batch first.</Link>
              </p>
            ) : (
              <select
                id="batchId"
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-700 bg-slate-800 px-4 py-2.5 text-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            />
            <p className="text-xs text-slate-500 mt-1">Leave blank or 0 for no expiration</p>
          </div>
          ...
          <div className="flex gap-3 pt-4">
            <Link
              href="/admin/enrollment-forms"
              className="flex-1 px-4 py-3 border border-slate-700 text-slate-300 font-semibold rounded-lg hover:bg-slate-800 transition-colors text-center"
            >
              Cancel
            </Link>
            <button
