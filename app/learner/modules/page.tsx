  ...
  const getStatusConfig = (mod: ModuleWithProgress) => {
    if (mod.isLocked) {
      return {
        ...
        cardClass: "border-slate-800 bg-slate-900/50 opacity-70",
        actionLabel: "Locked",
    ...
    if (status === "IN_PROGRESS" || status === "CONTENT_COMPLETED") {
      return {
        ...
        cardClass: "border-blue-900/40 bg-blue-950/10",
    ...
    if (status === "MCQ_AVAILABLE") {
      return {
        ...
        cardClass: "border-amber-900/40 bg-amber-950/10",
    ...
    if (status === "COMPLETED") {
      return {
        ...
        cardClass: "border-emerald-900/40 bg-emerald-950/10",
                        ...
                        {mod.contents && mod.contents.length > 0 && (
                          <div className="flex items-center gap-2 mb-3 flex-wrap">
                            {Array.from(new Set(mod.contents.map((c) => c.contentType))).map((type) => (
                              <span
                                key={type}
                                className="inline-flex items-center gap-1 text-xs bg-slate-800 text-slate-400 rounded px-2 py-0.5 border border-slate-700"
                              >
                                {type === "VIDEO" && "🎥"}
                                {type === "PDF" && "📄"}
                                {type === "PPT" && "📊"}
