    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-950 px-4">
        <div className="max-w-md w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 text-center">
          <div className="w-16 h-16 bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-500 rounded-full flex items-center justify-center mx-auto mb-4">
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 py-12 px-4">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 text-center shadow-xl shadow-slate-200/50 dark:shadow-black/50">
            <div className="w-20 h-20 bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl p-6">
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Score</p>
                <p className={`text-4xl font-bold ${result.passed ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}>
                  {result.percentage}%
                </p>
                <p className="text-sm text-slate-400 dark:text-slate-500 mt-2">{result.obtainedMarks} / {result.totalMarks} Marks</p>
              </div>
              <div className="bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl p-6 text-left space-y-3">
                <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700/50 pb-2">
                  ...
                  <span className="text-sm font-bold text-slate-900 dark:text-white">{result.totalQuestions}</span>
                </div>
                <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700/50 pb-2">
                  <span className="text-sm text-slate-500 dark:text-slate-400">Correct Answers</span>
                  <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">{result.correctAnswers}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-500 dark:text-slate-400">Incorrect/Unanswered</span>
                  <span className="text-sm font-bold text-red-600 dark:text-red-400">{result.incorrectAnswers}</span>
                </div>
              </div>
            </div>

            <div className="bg-blue-50 border border-blue-200 dark:bg-blue-500/10 dark:border-blue-500/20 rounded-xl p-4 mb-8">
              <p className="text-blue-700 dark:text-blue-300 font-medium">Module Status: {result.status}</p>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20">
        ...
        <div className="max-w-3xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0">
              {module && (
                <p className="text-xs font-medium text-slate-400 dark:text-slate-500 truncate">Module {module.sequenceOrder}</p>
              )}
          ...
          <div className="text-sm font-medium text-slate-500 dark:text-slate-400 shrink-0 ml-4">
      ...
      <div className="max-w-3xl mx-auto p-6 mt-6 space-y-8">
        {questions.length === 0 ? (
          <div className="text-center py-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
            <p className="text-slate-400 dark:text-slate-500">No questions available for this module.</p>
          </div>
        ) : (
          questions.map((q, index) => (
            <div key={q.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 shadow-sm">
              <div className="flex gap-4 mb-6">
                <div className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400 flex items-center justify-center font-bold">
                  {index + 1}
                </div>
                <div>
                  <h3 className="text-lg font-medium text-slate-900 dark:text-white leading-relaxed">{q.questionText}</h3>
                  <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">{q.marks} Marks</p>
                </div>
              </div>
                    ...
                    <label
                      ...
                      className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${
                        isSelected
                          ...
                          : "bg-slate-100/50 dark:bg-slate-800/30 border-slate-300/50 dark:border-slate-700/50 hover:border-slate-400 dark:hover:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800"
                      }`}
                    >
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                        isSelected ? "border-blue-500" : "border-slate-400 dark:border-slate-500"
                      }`}>
                        {isSelected && <div className="w-2.5 h-2.5 rounded-full bg-blue-500" />}
                      </div>
                      <span className={`text-base ${isSelected ? "text-slate-900 dark:text-white" : "text-slate-600 dark:text-slate-300"}`}>
                        {opt.optionText}
                      </span>
                    </label>
                  );
      ...
      {showWarning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-3xl p-8 shadow-2xl">
            <div className="w-16 h-16 bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-500 rounded-full flex items-center justify-center mb-6">
              <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-3">Unanswered Questions</h3>
            <p className="text-slate-500 dark:text-slate-400 mb-8">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 rounded-xl border border-slate-300 dark:border-slate-700 px-4 py-3 font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              >
                Go Back
              </button>
              <button 
