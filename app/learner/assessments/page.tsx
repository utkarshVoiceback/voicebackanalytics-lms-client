    ...
    switch (item.assessmentStatus) {
      ...
      case "NOT_STARTED":
        return (
          <button
            onClick={() => router.push(`/learner/modules/${item.moduleId}`)}
            className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-300 hover:bg-slate-800 transition-colors whitespace-nowrap"
          >
            Start Module
          </button>
        );
      default:
        return (
          <span className="rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-slate-500 cursor-not-allowed whitespace-nowrap">
            Locked
          </span>
        );
    }
    ...
    <div className="p-6 lg:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-400 mt-1">Track your assessment progress across all training modules.</p>
        </div>
        ...
        {!error && assessments.length === 0 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
            ...
            <h2 className="text-lg font-semibold text-white mb-2">No Assessments Available</h2>
            <p className="text-slate-400">Assessments will appear here as your training modules are set up.</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-emerald-950/30 border border-emerald-900/50 rounded-xl p-5">
                <p className="text-xs font-medium text-emerald-500/70 uppercase tracking-wider mb-1">Completed</p>
                <p className="text-2xl font-bold text-emerald-400">{completedCount}</p>
                <p className="text-xs text-emerald-500/70 mt-1">Passed</p>
              </div>
              {avgScore !== null ? (
                <div className="bg-blue-950/30 border border-blue-900/50 rounded-xl p-5 col-span-2 sm:col-span-1">
                  <p className="text-xs font-medium text-blue-500/70 uppercase tracking-wider mb-1">Avg Score</p>
                  <p className="text-2xl font-bold text-blue-400">{avgScore.toFixed(1)}%</p>
                  <p className="text-xs text-blue-500/70 mt-1">Across completed</p>
                </div>
              ) : availableCount > 0 ? (
                <div className="bg-amber-950/30 border border-amber-900/50 rounded-xl p-5 col-span-2 sm:col-span-1">
                  <p className="text-xs font-medium text-amber-500/70 uppercase tracking-wider mb-1">Available</p>
                  <p className="text-2xl font-bold text-amber-400">{availableCount}</p>
                  <p className="text-xs text-amber-500/70 mt-1">Ready to take</p>
                </div>
              ) : null}
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-800">
                    ...
                    <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-6 py-4">Questions</th>
                        ...
                        <td className="px-6 py-4">
                          {item.latestAttempt ? (
                            <div>
                              ...
                              <p className="text-xs text-slate-400">{item.latestAttempt.percentage.toFixed(1)}%</p>
                            </div>
                          ) : (
