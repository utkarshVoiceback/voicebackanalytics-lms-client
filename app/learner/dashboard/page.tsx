  ...
  const getStatusConfig = (mod: ModuleWithProgress) => {
    ...
    if (!status || status === "NOT_STARTED") {
      ...
      return {
        ...
        icon: (
          <svg className="w-6 h-6 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    ...
    if (status === "COMPLETED") {
      return {
        ...
        action: "Review",
        actionStyle: "bg-emerald-100 hover:bg-emerald-200 text-emerald-700 border border-emerald-300 dark:bg-emerald-700/40 dark:hover:bg-emerald-700/60 dark:text-emerald-300 dark:border-emerald-700",
      };
    }
    return {
      label: status,
      ...
      icon: null,
      action: "Open",
      actionStyle: "bg-slate-200 text-slate-900 dark:bg-slate-700 dark:text-white",
    };
  };
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Total Modules</p>
                <p className="text-3xl font-bold text-slate-900 dark:text-white">{modules.length}</p>
              </div>
            ...
            {modules.length > 0 && (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 mb-8">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-slate-900 dark:text-white">{Math.round((completedCount / modules.length) * 100)}%</span>
                </div>
            ...
            {modules.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center">
                <p className="text-slate-500 dark:text-slate-400">No modules have been assigned to your batch yet.</p>
              </div>
            ) : (
                    ...
                    <div key={mod.id} className={`border rounded-xl p-5 transition-all ${config.bg} ${config.border}`}>
                      <div className="flex items-center gap-5">
                        ...
                        <div className="flex-shrink-0 flex flex-col items-center gap-1">
                          <div className="flex items-center justify-center w-11 h-11 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-700 text-slate-500 dark:text-slate-400 text-sm font-bold">
                            {mod.sequenceOrder}
                          </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-3 mb-1 flex-wrap">
                            {config.icon}
                            <h3 className="text-base font-semibold text-slate-900 dark:text-white">{mod.title}</h3>
