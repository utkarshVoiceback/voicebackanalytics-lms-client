  ...
  const getStatusConfig = (mod: ModuleWithProgress) => {
    if (mod.isLocked) {
      return {
        ...
        actionLabel: "Locked",
        actionClass: "bg-slate-800 text-slate-500 cursor-not-allowed",
        icon: (
    ...
    if (status === "COMPLETED") {
      return {
        ...
        actionClass: "bg-emerald-700/30 hover:bg-emerald-700/50 text-emerald-300 border border-emerald-700/50",
        icon: (
    ...
    return {
      ...
      actionLabel: "Open",
      actionClass: "bg-slate-700 text-white",
      icon: null,
    };
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {!error && modules.length === 0 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
            ...
            <h2 className="text-lg font-semibold text-white mb-2">No Modules Available</h2>
            <p className="text-slate-400">Your batch has no training modules yet. Check back soon.</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-emerald-950/30 border border-emerald-900/50 rounded-xl p-5">
                ...
                <p className="text-xs text-emerald-500/70 mt-1">Modules</p>
              </div>
              <div className="bg-blue-950/30 border border-blue-900/50 rounded-xl p-5">
                <p className="text-xs font-medium text-blue-500/70 uppercase tracking-wider mb-1">In Progress</p>
                <p className="text-2xl font-bold text-blue-400">{inProgressCount + mcqAvailableCount}</p>
                <p className="text-xs text-blue-500/70 mt-1">Modules</p>
              </div>
              <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl p-5">
                <p className="text-xs font-medium text-slate-500/70 uppercase tracking-wider mb-1">Remaining</p>
                <p className="text-2xl font-bold text-slate-300">{remainingCount}</p>
                <p className="text-xs text-slate-500/70 mt-1">Modules</p>
              </div>
            </div>
            ...
            {totalPossible > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 mb-6">
                <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Total Marks</p>
                <p className="text-xl font-bold text-white">
                  {totalObtained}/{totalPossible}
                  {totalPossible > 0 && <span className="text-slate-400 text-sm ml-2">({Math.round((totalObtained/totalPossible)*100)}%)</span>}
                </p>
              </div>
            )}
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-shrink-0">
                        <div className="w-10 h-10 rounded-lg bg-slate-800/80 border border-slate-700 flex items-center justify-center text-sm font-bold text-slate-300">
                          {mod.sequenceOrder}
                        </div>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {mod.description && (
                          <p className="text-sm text-slate-400 mb-3 line-clamp-2">{mod.description}</p>
                        )}
