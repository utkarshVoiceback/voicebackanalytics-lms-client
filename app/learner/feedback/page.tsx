...
export default function LearnerFeedbackPage() {
  ...
  useEffect(() => {
    ...
    socket.on("message:new", (msg: Message) => {
      if (conversationRef.current && conversationRef.current.id === msg.conversationId) {
        setMessages((prev) => {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res: any = await apiFetch(`/conversations/${conversation.id}/messages`, {
        method: "POST",
  ...
  return (
    <div className="h-[calc(100vh-theme(spacing.16))] bg-slate-950 p-6 flex flex-col">
      <div className="mb-4">
        ...
        <p className="text-slate-400 mt-1">Communicate directly with your instructor.</p>
      </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-500 h-full">
               ...
               <p className="text-lg font-medium text-slate-400 mb-1">No feedback messages yet.</p>
