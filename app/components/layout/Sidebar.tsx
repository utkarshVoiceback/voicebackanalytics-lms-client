...
const adminNav: { main: NavItem[]; sections: NavSection[] } = {
  main: [
    { href: "/admin/dashboard", label: "Dashboard", icon: icons.dashboard, exact: true },
  ],
  sections: [
    {
      title: "HOME PAGE",
...
const learnerNav: { main: NavItem[]; sections: NavSection[] } = {
  ...
  sections: [
    ...
    // },
    {
      title: "Communication",
      items: [
        { href: "/learner/notifications", label: "Notifications", icon: icons.notifications },
      ...
      <div className="flex-shrink-0 border-t border-slate-800 p-3">
        {collapsed && !isMobile ? (
          ...
          <div className="space-y-2">
            ...
            >
              <span className="w-4 h-4 group-hover:text-red-400 text-slate-500">{icons.logout}</span>
              Logout
            </button>
          </div>
        )}
