          ...
          <div className="p-1">
            {filteredOptions.length === 0 ? (
              ...
              Object.entries(groupedOptions).map(([groupName, groupItems]) => (
                <div key={groupName}>
                  {groupName !== "Other" && (
                    <div className="px-3 py-2 text-xs font-semibold text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/30 border-b border-slate-200 dark:border-slate-700">
                      {groupName}
                    </div>
                  )}
                  {groupItems.map(opt => (
                    <div
                      key={opt.id}
                      className={`flex items-center px-3 py-2 text-sm rounded cursor-pointer transition-colors ${groupName !== "Other" ? "ml-2" : ""} ${selectedIds.includes(opt.id) ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300'}`}
